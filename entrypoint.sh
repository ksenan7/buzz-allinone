...
# Logs go to STDOUT (so container/log platforms capture them).
# No `set -e`: we log each step and continue; only the final relay must run.
set -x
...
echo "[entrypoint] launching relay in foreground (logs now on stdout)..."
exec buzz-relay
